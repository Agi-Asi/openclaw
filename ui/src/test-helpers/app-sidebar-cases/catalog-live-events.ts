  ...
  it("ignores browser presence churn when deciding whether native catalogs changed", async () => {
    ...
    try {
      ...
      gateway.publishEvent("presence", {
        presence: [{ deviceId: "browser-1", mode: "webchat", roles: ["node"], reason: "connect" }],
      });
      gateway.publishEvent("presence", {
        presence: [
          { deviceId: "browser-1", mode: "webchat", roles: ["node"], reason: "connect" },
          { deviceId: "browser-2", mode: "webchat", roles: ["node"], reason: "connect" },
        ],
      });
