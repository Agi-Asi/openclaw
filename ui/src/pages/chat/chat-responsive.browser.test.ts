...
describeBrowserLayout.concurrent("chat responsive browser layout", () => {
  ...
  it("keeps the expanded mobile composer tight, scrollable, and flush with the thread", async () => {
    ...
    try {
      ...
      expect(attach.x + attach.width).toBeLessThanOrEqual(context.x + 1);
      expect(context.x + context.width).toBeLessThanOrEqual(model.x + 1);
    ...
    async (width, height) => {
      ...
      try {
        ...
        if (width <= 480) {
          ...
          expect(attach.x + attach.width).toBeLessThanOrEqual(context.x + 1);
          expect(context.x + context.width).toBeLessThanOrEqual(modelSettings.x + 1);
        } else {
