...
function openChatComposerPickers(root: ParentNode = document): HTMLElement[] {
  return Array.from(
    root.querySelectorAll<HTMLElement>(".agent-chat__input details, .agent-chat__input wa-dropdown"),
...
export function restorePointerOpenedChatComposerTrigger(event: Event): void {
  ...
  if (
    ...
    const clearPointerFocus = () =>
      trigger.removeAttribute(POINTER_RESTORED_FOCUS_ATTRIBUTE);
...
export function closeChatComposerPickerOnEscape(event: KeyboardEvent): void {
  if (event.key !== "Escape") {
    return;
  }
  ...
  const picker = Array.from(
    composer.querySelectorAll<HTMLElement>("details[open], wa-dropdown[open]"),
  ).at(-1);
  if (!picker) {
    return;
  }
  ...
  event.stopPropagation();
  closeComposerPicker(picker);
  pickerTrigger(picker)?.focus({ preventScroll: true });
}
...
export function closeChatDetailsOnEscape(event: KeyboardEvent): boolean {
  if (event.key !== "Escape") {
    return false;
  }
  const details = event.currentTarget;
