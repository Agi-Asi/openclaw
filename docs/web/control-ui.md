...
Sessions that the Gateway can classify show a spinning run indicator while working and an animated hourglass while waiting for execution capacity. The open chat mirrors the wait with elapsed time and any available queue-position and slot facts. A terminal run error remains visible after reload, so a user-only final message does not look silently abandoned.
