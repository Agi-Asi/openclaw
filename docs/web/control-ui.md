...
When you start a local managed-worktree session, the session opens immediately with the first message and a checkout card. The card streams setup progress and offers **Cancel** or **Work locally** until the worktree is ready; the agent starts only after that setup finishes or you choose the existing workspace.
