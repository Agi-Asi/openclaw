...
<AccordionGroup>
  <Accordion title="Send and history semantics">
    ...
    - On the New Session page, press **Cmd+Enter** on macOS or **Ctrl+Enter** elsewhere to create and start the draft as a background session without leaving the page. The existing Chat shortcut is unchanged: during an active run, Cmd/Ctrl+Enter still steers the current session. A background session appears in the sidebar and shows an actionable top-right notice when its first turn finishes.
