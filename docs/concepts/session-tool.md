...
- `action: "patch"` can also mark a visible session read or unread. Existing category, icon, lifecycle, model, and thinking settings remain available.
- `action: "patch_many"` changes category and/or unread state for up to 100 visible `targets`. Each target accepts `sessionKey` and optional `expectedSessionId`. Results return requested/updated counts plus bounded failures instead of repeating every successful key.
