...
- `channels.slack.thread.historyScope` default is `thread`. When `thread.inheritParent` is unset, bot-opened threads inherit the parent channel transcript while existing user-created threads stay isolated. Set it to `true` to inherit all threads or `false` to isolate all threads.
