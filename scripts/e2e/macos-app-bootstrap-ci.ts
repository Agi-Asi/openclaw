...
const productHead = "4ac6a8448e30397edf33eeb3f7d5f82f11998df9";
...
class LiveProof {
  ...
  private async launchAndNavigate(): Promise<void> {
    ...
    await delay(2_000);
    await this.captureWindow("01-welcome.png");
    await this.clickNext();
    await delay(2_000);
    await this.captureWindow("02-connection.png");
    await this.clickNext();
  }

  private async waitForFailedServiceState(): Promise<string> {
    await this.waitFor("managed candidate CLI", 30_000, () =>
      existsSync(path.join(this.stateDir, "bin/openclaw")),
    );
    await this.waitFor("failing Gateway wrapper invocation", 180_000, () =>
      existsSync(path.join(this.artifactDir, "gateway-wrapper.log")),
    );
    // SwiftUI static text is visible but not exposed by `entire contents` on the
    // hosted runner. Give the failure state time to render, then preserve the
    // best-effort tree and validate the actual checklist from the screenshot.
    await delay(4_000);
    return await this.uiDump();
  }
