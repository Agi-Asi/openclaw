...
describe("transcript memory policy companions", () => {
  ...
  it("does not pass a pending transcript event to manual compaction", async () => {
    ...
    await expect(trimTranscriptForManualCompact(scope(env), selectRetainedLines)).resolves.toEqual({
      trimmed: false,
    });
    ...
    await withOwnedSessionTranscriptWrites(
      ...
      async () => {
        ...
        for (const type of ["tool-result", "summary", "checkpoint", "system"] as const) {
          await appendTranscriptEvent(scope(env), { type, value: `${type} event` });
        }
      },
    );
  ...
  it("preserves only exact same-session replacement lineage and leaves new rows pending", async () => {
    ...
    const original = loadTranscriptEventsSync(scope(env));
    ...
    await replaceTranscriptEvents(scope(env), [retainedMessage!]);
    ...
    await withOwnedSessionTranscriptWrites(
      ...
      async () => {
        await replaceTranscriptEvents(scope(env), [
          {
            message: {
              role: "assistant",
