  ...
  it("rejects retained destructive repair callbacks after their owner expires", async () => {
    await withOpenClawTestState(
      ...
      async ({ env }) => {
        ...
        const observed = context.readPluginStateEntriesInKeyRange?.(options, {
          prefix: "binding:",
          limit: 10,
        });
        ...
        expect(() =>
          context.deletePluginStateEntriesIfUnchanged?.(options, observed ?? []),
        ).toThrow("repair owner expired");
        expect(() =>
          context.readPluginStateEntriesInKeyRange?.(options, { prefix: "binding:", limit: 10 }),
        ).toThrow("repair owner expired");
        await expect(
          context.readSessionIdentityEvidenceBatch?.([{ agentId: "main", sessionId: "gone" }]),
