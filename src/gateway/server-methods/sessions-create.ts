...
import { createSessionInitialDispatch } from "./session-create-initial-dispatch.js";
import {
...
import {
  buildInitializingWorktreeStartup,
  startSessionWorktreeStartup,
  type SessionWorktreeStartupRuntime,
} from "./session-worktree-startup.js";
import { sessionLog } from "./sessions-shared.js";
import { sessionsStartupResolveHandler } from "./sessions-startup-resolve.js";
...
export const sessionCreateHandlers: GatewayRequestHandlers = {
  "sessions.create": async ({
    ...
    const startupRuntime: SessionWorktreeStartupRuntime = { commitGuard };
    ...
    if (p.worktree === true) {
      ...
      prepareLifecycle = async (lifecycleTarget) => {
        let provisionedWorktree: Awaited<ReturnType<typeof managedWorktrees.create>> | undefined;
        try {
          ...
          if (existing && existingDirectory) {
            ...
            sessionWorktree = await managedWorktrees.create({
              ...
              onStage: async (stage) => {
                await startupRuntime.reportStage?.(stage);
              },
              onOutput: (chunk) => startupRuntime.reportOutput?.(chunk),
              ...(startupRuntime.control?.signal ? { signal: startupRuntime.control.signal } : {}),
              ...(startupRuntime.commitGuard ? { commitGuard: startupRuntime.commitGuard } : {}),
            });
            provisionedWorktree = sessionWorktree;
          }
          ...
          if (provisionedWorktree) {
            await managedWorktrees.remove({
              id: provisionedWorktree.id,
              reason: "session-create-failed",
              allowSnapshotLoss: true,
            });
          }
    ...
    const initialDispatch = createSessionInitialDispatch({
      active: authority.hasActive,
      attachments: initialAttachments,
      client,
      context,
      enabled: hasInitialTurn,
      isWebchatConnect,
      message: initialMessage,
      req,
    ...
    const sessionStartupState = buildInitializingWorktreeStartup({
      attachments: initialAttachments,
      message: initialMessage,
    ...
    const created = await createGatewaySession({
      ...
      afterCreate: startupOperationId ? undefined : initialDispatch.afterCreate,
    ...
    if (startupOperationId && prepareLifecycle) {
      await startSessionWorktreeStartup({
        cfg,
        client,
        commitGuard,
        context,
        ...
        sourceCwd: startupSourceCwd,
        sourceRoot: startupSourceRoot,
        startedAt: startupStartedAt,
        startupOperationId,
        titleModelTarget: catalogTarget?.target ?? requestedModel,
        worktreeTitle: () => worktreeTitle,
      });
      return;
    }
    if (created.postCommit.status === "failed") {
      initialDispatch.result.runError = errorShape(
        ErrorCodes.UNAVAILABLE,
        formatErrorMessage(created.postCommit.error),
      );
    }
    ...
    const { messageSeq, runError, runMeta, runPayload } = initialDispatch.result;
    const runStarted =
  ...
  "sessions.startup.resolve": sessionsStartupResolveHandler,
};
