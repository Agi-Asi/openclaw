...
async function deliverBoundApprovalWebPush<TPayload>(params: {
  ...
  const deliveredSubscriptions = subscriptions.filter((subscription) =>
    deliveredSubscriptionIds.has(subscription.subscriptionId),
  );
  return deliveredSubscriptions.length > 0
    ? {
        cfg: params.cfg,
...
export function createApprovalWebPushDelivery(params: {
  ...
  const handleTerminal = async (approval: { id: string }): Promise<void> => {
    const deliveryState = deliveriesByApprovalId.get(approval.id);
    deliveriesByApprovalId.delete(approval.id);
    if (!deliveryState) {
      return;
    }
    const requestDelivery = await deliveryState.requestPushPromise;
    if (!requestDelivery) {
      return;
    }
    const results = await requestDelivery.sender({
      subscriptions: requestDelivery.subscriptions,
      payload: {
        title: "OpenClaw approval updated",
        body: "This approval is no longer pending.",
        renotify: false,
        tag: approvalWebPushTag(approval.id),
        url: approvalWebPushUrl(requestDelivery.cfg, approval.id),
      },
    ...
    if (!results.some((result) => result.ok)) {
      params.log?.warn?.(
        `approval Web Push terminal replacement reached no browsers approvalId=${approval.id}`,
      );
    }
  };
  ...
  return {
    ...
    handleRequested<TPayload>(record: ExecApprovalRecord<TPayload>): boolean | Promise<boolean> {
      if (listBoundWebPushSubscriptions().length === 0) {
        return false;
      }
