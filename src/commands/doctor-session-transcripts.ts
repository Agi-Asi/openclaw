...
async function noteSessionSqliteMigrationHealth(params: {
  ...
  const runSessionSqlite = async (maintenanceAuthority?: DoctorSqliteMaintenanceAuthority) => {
    ...
    if (params.shouldRepair) {
      if (!maintenanceAuthority) {
        throw new Error(
          "Post-session plugin repair requires current SQLite maintenance ownership.",
        );
      }
      ...
      const messages = [...pluginRepair.changes, ...pluginRepair.warnings];
